...
struct Mc6850 {
    void reset();
    bool isSelected(uint16_t addr) const { return _enabled && (addr & ~1) == _baseAddr; }
...
private:
    uint8_t _control;
    uint8_t _status;
    static constexpr uint8_t rxIrq = (1 << 0);
    static constexpr uint8_t txIrq = (1 << 1);
    uint8_t _irq;
    bool _enabled;
    uint16_t _baseAddr;
    uint8_t _txData;
    ...
    uint8_t txReady() const { return _status & TDRE_bm; }
    uint8_t rxReady() const { return _status & RDRF_bm; }
    void assertRxIntr();
    void negateRxIntr();
    void assertTxIntr();
    void negateTxIntr();
